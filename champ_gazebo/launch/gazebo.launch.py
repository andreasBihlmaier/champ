...
def generate_launch_description():
    ...
    default_links_map_path = os.path.join(
        config_pkg_share, "config/links/links.yaml")
    declare_links_map_path = DeclareLaunchArgument(
        name="links_map_path", default_value=default_links_map_path, description="Absolute path to robot links.yaml file")
    ...
    start_gazebo_server_cmd = ExecuteProcess(
        cmd=[
            ...
            # '-e',
            # 'ode',
            '--pause',  # TODO depend on pause argument
            "-s",
    ...
    robot_description = {"robot_description": Command(
        ["xacro ", LaunchConfiguration("description_path"), " use_sim:=true"])}
